...
def SLIDE_INSTRUCTIONS(topic: str) -> str:
    # No device configs, no packet walks -- those belong to the networking
    # certs. CISSP is explicitly a management-level exam ("think like a
    # manager, CEO or owner"), and Security+ and CC both test the same
    # governance/risk reasoning at a shallower depth, so the instruction asks
    # for a decision scenario instead of a technical walkthrough: what a
    # security leader is choosing between, and why one option wins.
    return (
        ...
        f"3) สถานการณ์จำลองระดับผู้บริหาร (management scenario) อย่างน้อย 1 หน้าเต็ม: "
        f"ตั้งสถานการณ์ที่ต้องเลือกระหว่างตัวเลือกที่เป็นไปได้จริง 2-3 ทาง แล้วอธิบายว่าทางไหนคือคำตอบที่ "
        f"CISSP ควรเลือกและเพราะอะไร (มองจากมุมลดความเสี่ยงให้องค์กรอย่างคุ้มค่า ไม่ใช่มุมเทคนิคล้วน) "
        f"4) ถ้าหัวข้อมีตัวเลขหรือสูตรคำนวณ (เช่น ALE, RTO/RPO, CVSS) ให้ใส่ตัวอย่างคำนวณจริงพร้อมค่าจำลอง "
        f"ทีละขั้น ไม่ใช่แค่สูตร "
        f"5) Key points ที่ต้องจำสำหรับสอบ โดยเฉพาะคำที่ข้อสอบมักใช้ (MOST, BEST, FIRST) เทียบกับตัวเลือกอื่น "
        f"ข้อควรระวังสำคัญ: ห้ามยกตัวอย่าง config อุปกรณ์เครือข่าย (Cisco IOS หรืออื่นๆ) หรือ diagram เส้นทาง "
        f"ข้อมูลผ่าน router/switch เด็ดขาด เพราะเนื้อหานี้เป็นการบริหารความมั่นคงสารสนเทศ ไม่ใช่วิชาเครือข่าย "
        f"ตรวจสอบว่าทุกตัวอย่างสื่อสารตรงกับข้อความที่อธิบายจริง"
    )


def SUMMARY_PROMPT(topic: str) -> str:
    # Mirrors slides_v2's SLIDE_INSTRUCTIONS above, and exists for the same
    # reason: this cert used to inherit summary_only.py's hardcoded
    # networking prompt (section 3 explicitly asked for "Cisco IOS / IOS-XE"
    # config), which put a fabricated router-config section into 354 of the
    # first 357 summaries generated -- including topics like "Confidentiality"
    # that have nothing to do with routers. Section 3 here is a management
    # decision scenario instead of a device configuration.
...
1. แนวคิดหลักและความสำคัญ — อธิบายว่ามันคืออะไร ใช้แก้ปัญหาอะไร และทำไมข้อสอบถึงถามเรื่องนี้ (ไม่ใช่แค่นิยาม ต้องบอกว่ามันสำคัญต่อการบริหารความเสี่ยงขององค์กรอย่างไร)
...
3. สถานการณ์จำลองระดับผู้บริหาร (management scenario) — ตั้งสถานการณ์ที่ต้องเลือกระหว่างตัวเลือกที่เป็นไปได้จริง 2-3 ทาง แล้วอธิบายว่าทางไหนคือคำตอบที่ CISSP ควรเลือกและเพราะอะไร (มองจากมุมลดความเสี่ยงให้องค์กรอย่างคุ้มค่า ไม่ใช่มุมเทคนิคล้วน) ถ้าหัวข้อมีตัวเลขหรือสูตรคำนวณ (เช่น ALE, RTO/RPO, CVSS) ให้ใส่ตัวอย่างคำนวณจริงพร้อมค่าจำลองทีละขั้นแทนสถานการณ์นี้
4. ข้อแตกต่าง/เปรียบเทียบกับแนวคิดใกล้เคียงที่ข้อสอบชอบเอามาสับสนกัน (ถ้ามี) — เช่น ถ้าเป็น access control model ให้เทียบกับโมเดลอื่นในกลุ่มเดียวกัน
5. ข้อผิดพลาดที่พบบ่อย (Common pitfalls) ที่มักทำให้ติดตอนสอบหรือตอนทำงานจริง โดยเฉพาะคำที่ข้อสอบมักใช้ (MOST, BEST, FIRST) เทียบกับตัวเลือกอื่นที่ดูถูกแต่ไม่ใช่คำตอบที่ดีที่สุด
...
- ที่ไหนมีค่าตัวเลขหรือตัวเปรียบเทียบหลายตัว (เช่น domain weight, RTO/RPO, ALE) ให้ใช้ตาราง Markdown แทนการร่ายเป็นย่อหน้า
- **ห้ามยกตัวอย่าง config อุปกรณ์เครือข่าย (Cisco IOS หรืออื่นๆ) หรือ fenced code block แบบคำสั่งอุปกรณ์เด็ดขาด** เพราะเนื้อหานี้เป็นการบริหารความมั่นคงสารสนเทศ ไม่ใช่วิชาเครือข่าย
